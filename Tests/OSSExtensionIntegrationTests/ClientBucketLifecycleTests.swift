...
final class ClientBucketLifecycleTests: BaseTestCase {
    ...
    func testPutAndGetBucketLifecycleSuccess() async throws {
        ...
        let rules = [LifecycleRule(id: "rule1",
                                   status: "Enabled",
                                   prefix: "1prefix/",
                                   transitions: [Transition(days: 30,
                                                            storageClass: "Archive")],
                                   noncurrentVersionTransitions: [NoncurrentVersionTransition(noncurrentDays: 30,
                                                                                              storageClass: "IA")],
                                   expiration: Expiration(days: 180),
                                   abortMultipartUpload: AbortMultipartUpload(days: 30),
                                   noncurrentVersionExpiration: NoncurrentVersionExpiration(noncurrentDays: 180)),
                     LifecycleRule(id: "rule2",
                                   status: "Disabled",
                                   prefix: "2prefix/",
                                   transitions: [Transition(createdBeforeDate: formatter.date(from: "2025-05-05T00:00:00.000Z"),
                                                            storageClass: "IA")],
                                   noncurrentVersionTransitions: [NoncurrentVersionTransition(noncurrentDays: 30,
                                                                                              storageClass: "IA")],
                                   expiration: Expiration(createdBeforeDate: formatter.date(from: "2025-04-05T00:00:00.000Z")),
                                   abortMultipartUpload: AbortMultipartUpload(createdBeforeDate: formatter.date(from: "2025-05-05T00:00:00.000Z")),
                                   noncurrentVersionExpiration: NoncurrentVersionExpiration(noncurrentDays: 180))]
