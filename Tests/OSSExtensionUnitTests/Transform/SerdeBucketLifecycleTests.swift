...
class SerdeBucketLifecycleTests: XCTestCase {
    ...
    func testDeserializeGetBucketLifecycle() {
        ...
        XCTAssertEqual(result.lifecycleConfiguration?.rules?.first?.filter?.nots?.first?.prefix, "prefix1")
        XCTAssertEqual(result.lifecycleConfiguration?.rules?.first?.filter?.nots?.first?.tag?.key, "key2")
        XCTAssertEqual(result.lifecycleConfiguration?.rules?.first?.filter?.nots?.first?.tag?.value, "value2")
